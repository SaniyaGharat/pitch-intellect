  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Tracking Quality — Ball-Free Game State Reconstruction" },
      {
        ...
        content: "Missingness, completeness, coordinate validity and jitter for the active dataset.",
      },
    ],
  }),
  ...
  return (
    <>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-6">
        <Metric label="Missingness Rate" value={`${(TRACKING_QUALITY.missingness * 100).toFixed(1)}%`} tone="warn" />
        <Metric label="Player Completeness" value={`${(TRACKING_QUALITY.completeness * 100).toFixed(1)}%`} />
        <Metric label="Coordinate Validity" value={`${(TRACKING_QUALITY.validity * 100).toFixed(1)}%`} tone="primary" />
        ...
        <Metric label="Mean Gap Length" value={`${TRACKING_QUALITY.meanGapLen} f`} sub="≈0.30 s at 25 FPS" />
      </div>
