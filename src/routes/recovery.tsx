...
export const Route = createFileRoute("/recovery")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Recovery & Mitigation — Ball-Free Game State Reconstruction" },
      {
...
const STAGES = [
  {
    key: "reference",
    name: "Reference",
    detail: "Clean Metrica tracking, 22 tracks, no injected degradation. Baseline for all comparisons.",
  },
  {
    key: "degraded",
    name: "Degraded",
    detail: "15% player dropout and 0.5 m coordinate jitter applied. Support graph is the first stage to fail.",
  },
  {
    key: "interpolated",
    name: "Linear Interpolation",
    detail: "Gaps shorter than 12 frames filled linearly. Cheap, but produces implausible straight-line motion.",
  },
  {
    key: "kalman",
    name: "Kalman Filter",
    detail: "Constant-velocity model with per-track process noise. Best recovery, materially higher runtime.",
  },
];
  ...
  return (
    <>
            ...
            <div key={s.key} className="flex items-center gap-2">
              <button
                ...
                className={`border px-3 py-2 text-left text-[12px] ${
                  stage === i ? "border-primary bg-accent text-foreground" : "border-border text-muted-foreground"
                }`}
              >
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
        <Panel title="Comparative Metrics" subtitle="Each stage against the reference configuration">
          ...
          <div className="mt-3 border border-border bg-panel-alt p-2.5 text-[11.5px] leading-relaxed text-muted-foreground">
            Kalman filtering recovers 0.054 F1 of the 0.089 lost to degradation — approximately 61% — at a
            74% runtime increase over the degraded pipeline. Interpolation recovers 40% at negligible cost.
          </div>
        </Panel>
