    ...
    <>
      ...
      <Panel title="Noise Parameters" className="mb-4">
        <div className="grid gap-4 md:grid-cols-4">
          {[
            { label: "Missing detections (%)", v: missing, set: setMissing, min: 0, max: 40, step: 1 },
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Metric label="Projected Possession F1" value={f1.toFixed(3)} tone={f1 < 0.6 ? "warn" : "primary"} />
      ...
      <Panel title="Impact Preview" subtitle="Fitted metric response across the swept severity range" className="mt-4">
        <LineSeries
          data={curve}
          xKey="severity"
          series={[
        ...
        <p className="mt-2 text-[11px] text-muted-foreground">
          Current composite severity: <span className="num">{severity.toFixed(3)}</span>. Projections are
          extrapolations of the benchmark fit, not fresh evaluations.
        </p>
      </Panel>
    </>
  );
