...
function Tactical() {
  ...
  const series = Array.from({ length: 40 }, (_, i) => {
    ...
    const ps = buildFrame(f);
    const h = teamGeometry(ps, "home");
    const a = teamGeometry(ps, "away");
    return {
      frame: f,
      advantage: +((h.compactness - a.compactness) * 2 + 0.02 * Math.sin(f / 60)).toFixed(3),
      homeWidth: h.width,
      awayWidth: a.width,
      homeLength: h.length,
      awayLength: a.length,
    ...
    <>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-5">
        <Metric
          ...
          tone="primary"
        />
        <Metric label="Mean Pressure (Home)" value={avg(homePlayers.map((p) => p.pressure)).toFixed(3)} />
        <Metric label="Mean Support (Home)" value={avg(homePlayers.map((p) => p.support)).toFixed(3)} />
        <Metric label="Home Compactness" value={home.compactness.toFixed(3)} sub={`Spread ${home.spread} m`} />
        <Metric label="Away Compactness" value={away.compactness.toFixed(3)} sub={`Spread ${away.spread} m`} />
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <Panel title="Tactical Advantage" subtitle="Rolling window, ±800 frames around current position">
          <LineSeries
            data={series}
            xKey="frame"
        ...
        <Panel title="Defensive Spread & Compactness" subtitle="Current frame comparison">
          <BarSeries
            data={[
              ...
              { metric: "Compact ×50", home: +(home.compactness * 50).toFixed(1), away: +(away.compactness * 50).toFixed(1) },
            ]}
            xKey="metric"
            series={[
      ...
      <Panel title="Per-Player Tactical Table" subtitle="Away side shown for comparison" className="mt-4">
