...
export const Route = createFileRoute("/broadcast")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Integration pending; modules disabled and unverified." },
    ],
  }),
  component: Broadcast,
});
...
const MODULES = [
  { name: "Frame Ingest", detail: "Broadcast video decode at 25 FPS", reason: "No verified video source in this build." },
  { name: "Player Detection", detail: "Per-frame bounding boxes", reason: "Detector weights not evaluated on this dataset." },
  { name: "Re-Identification", detail: "Identity persistence across cuts", reason: "No labelled re-ID benchmark available." },
  { name: "Camera Calibration", detail: "Homography to pitch coordinates", reason: "Calibration accuracy unmeasured; would silently bias all geometry." },
  { name: "Pitch Registration", detail: "Line-based keypoint fitting", reason: "Depends on calibration module." },
  { name: "Coordinate Export", detail: "Tracking stream to reconstruction pipeline", reason: "Blocked by upstream modules." },
];
  ...
  return (
    <>
      ...
      <div className="mb-4 border border-destructive/40 bg-destructive/5 px-3 py-2.5 text-[11.5px] leading-relaxed text-destructive">
        <span className="mr-1.5 font-semibold uppercase tracking-[0.06em]">Blocked — integration pending</span>
        No broadcast pipeline output has been validated. Any figure produced by these modules must be
        treated as unverified and must not be reported alongside tracking-based results.
      </div>
      ...
      <Panel title="Conditions for Enabling" className="mt-4">
        <ol className="list-decimal space-y-1.5 pl-4 text-[11.5px] leading-relaxed text-muted-foreground">
          <li>Calibration error measured against surveyed pitch points, reported with a confidence interval.</li>
          <li>Re-identification evaluated on a labelled broadcast segment with identity-switch counts.</li>
          <li>End-to-end positional error benchmarked against synchronised optical tracking.</li>
          <li>Only then may broadcast-derived state feed the reconstruction pipeline, clearly labelled as such.</li>
        </ol>
      </Panel>
    </>
  );
