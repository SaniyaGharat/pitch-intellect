  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Match Explorer — Ball-Free Game State Reconstruction" },
      {
        ...
        content: "Frame playback, event markers and player inspection on player-only tracking data.",
      },
    ],
  }),
...
function MatchExplorer() {
  const { frame, setFrame, dataset, setDataset, selectedPlayer, setSelectedPlayer } = useSession();
  ...
  useEffect(() => {
    if (!playing) return;
    const id = setInterval(() => setFrame(frame + Math.max(1, Math.round(speed * 2))), 80 / speed);
    ...
    <>
            ...
            {SPEEDS.map((s) => (
              <button
                ...
                className={`num border px-2 py-1 text-[11.5px] ${
                  speed === s ? "border-primary text-primary" : "border-border text-muted-foreground"
                }`}
              >
                {s}x
              </button>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.55fr_1fr]">
        <div className="space-y-4">
          ...
          <Panel title="Timeline" subtitle="Frames 1000–1500 · inferred possession bands and event markers">
            <div className="relative h-7 w-full border border-border bg-panel-alt">
              {POSSESSION_SEGMENTS.filter((s) => s.end > windowStart && s.start < windowEnd).map((s) => (
                <div
                  key={s.start}
        ...
        <Panel title="Player Inspector" subtitle={`#${sel.number} ${sel.name} · ${sel.role}`}>
          <dl className="text-[12px]">
            {[
              ...
              [
                "Heading",
                `${((Math.atan2(sel.vy, sel.vx) * 180) / Math.PI).toFixed(0)}°`,
              ],
              [
                "Nearest opponent",
                `${dist(sel, nearest(sel, players.filter((p) => p.team !== sel.team))).toFixed(1)} m`,
              ],
              [
                "Nearest teammate",
                `${dist(sel, nearest(sel, players.filter((p) => p.team === sel.team))).toFixed(1)} m`,
          ...
          <p className="mt-3 text-[11px] leading-relaxed text-muted-foreground">
            Pressure and support are model-derived quantities computed from relative player geometry
            only. They are not measurements and carry the uncertainty reported on the Game State view.
          </p>
        </Panel>
      </div>
    </>
