...
const METHODS = [
  { method: "Nearest-player heuristic (baseline)", poss: 0.612, event: 0.481, mrr: 0.402, note: "No learning; surprisingly strong on possession." },
  { method: "Voronoi control baseline", poss: 0.658, event: 0.502, mrr: 0.437, note: "Better in open play, worse in crowded areas." },
  { method: "Geometric posterior (ours)", poss: 0.741, event: 0.612, mrr: 0.583, note: "Reference configuration." },
  { method: "Geometric posterior + Kalman", poss: 0.706, event: 0.571, mrr: 0.512, note: "Only beneficial on degraded input." },
  { method: "Ball-informed upper bound", poss: 0.918, event: 0.847, mrr: 0.771, note: "Uses ball position; not comparable, shown as ceiling." },
];
  ...
  return (
    <>
      ...
      <Panel title="Benchmark Summary" className="mb-4">
        <BarSeries
          data={METHODS.map((m) => ({ method: m.method.split(" ")[0]!, poss: m.poss, event: m.event, mrr: m.mrr }))}
          xKey="method"
          series={[
      ...
      <Panel title="Negative and Inconclusive Findings" subtitle="Reported in full; not filtered for favourable outcomes">
        <ul className="list-disc space-y-2 pl-4 text-[11.5px] leading-relaxed text-muted-foreground">
          <li>
            The nearest-player heuristic reaches 0.612 possession F1 — 83% of our method's score — so most
            of the gain comes from a small number of contested frames, not from general modelling.
          </li>
          <li>
            Duel detection (F1 0.366) is no better than chance-adjusted marking detection. We do not claim
            duel recognition is feasible without ball information.
          </li>
          <li>
          ...
          <li>
            Counterfactual ranking has no ground truth. MRR is computed against annotated actual passes,
            which measures agreement with observed choices, not decision quality.
          </li>
          <li>
            Results on Synthetic_Controlled_A do not transfer cleanly to Metrica data; the fragmentation
            sweep shows a 0.04–0.07 F1 gap that we cannot currently explain.
          </li>
        </ul>
      </Panel>
    </>
